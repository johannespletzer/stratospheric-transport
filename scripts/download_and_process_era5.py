...
def main(
    start_year: int, end_year: int, download_dir: str, file_limit: int | None = None
) -> None:
    ...
    - Optionally limits the number of files downloaded per year with ``--file-limit``.
    """
    ...
    for i, year in enumerate(selected_years, start=1):
        ...
        if filtered_file_urls:
            ...
            download_files_multithreaded(
                filtered_file_urls, filtered_local_paths, max_workers=8
            )
        else:
    ...
    dataset = concatenate_files(
        download_dir, allowed_years=[str(y) for y in selected_years]
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download and process ERA5 tropopause data."
    )
    parser.add_argument(
        "--start-year",
        type=int,
        required=True,
        help="Start year of data to download (e.g., 2000)",
    )
    parser.add_argument(
        "--end-year",
        type=int,
        required=True,
        help="End year of data to download (e.g., 2020)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=DEFAULT_DOWNLOAD_DIR,
        help="Directory to store downloaded files and output file",
    )
    parser.add_argument(
        "--file-limit",
        type=int,
        default=None,
        help="Maximum number of files to download per year (for testing)",
    )

    args = parser.parse_args()
    main(args.start_year, args.end_year, args.output, args.file_limit)
